...
public class CMDashboardController {
	...
	public String cmDashboard(Model model, HttpSession session) {
		...
		try {
			CmDashboardModel[] region = restTemplate.getForObject(env.getUserUrl() + "cmDashboardGetIdds?id=" + userId1,
			...
			model.addAttribute("logo", cmDashboardGetIdd.get(0).getLogo());
			model.addAttribute("cm", cmDashboardGetIdd.get(0).getImage());
			model.addAttribute("name", cmDashboardGetIdd.get(0).getName());
	...
	public @ResponseBody JsonResponse<Object> getPatientNo(Model model, @RequestParam("state") String encodestate,
		...
		try {
			resp = restTemplate.getForObject(
					env.getUserUrl() + "genderWise-patientnocm?state=" + state + "&date=" + date + "&type=" + type,
					JsonResponse.class);
	...
	public @ResponseBody JsonResponse<Object> getAreaPatientNo(Model model, @RequestParam("state") String encodestate,
		...
		try {
			resp = restTemplate.getForObject(
					env.getUserUrl() + "areWise-patientnocm?state=" + state + "&date=" + date + "&type=" + type,
					JsonResponse.class);
	...
	public @ResponseBody JsonResponse<Object> getAgePatientNo(Model model, @RequestParam("state") String encodestate,
		...
		try {
			resp = restTemplate.getForObject(
					env.getUserUrl() + "ageWise-patientnocm?state=" + state + "&date=" + date + "&type=" + type,
					JsonResponse.class);
	...
	public @ResponseBody JsonResponse<Object> getDiseaseMaleFemalePatientNo(Model model, @RequestParam("state") String encodestate,
		...
		try {
			resp = restTemplate.getForObject(
					env.getUserUrl() + "diseaseWise-patientnocm?state=" + state + "&date=" + date + "&type=" + type,
					JsonResponse.class);
	...
	//High Risk Data Location Wise
			...
			//	System.out.println(state+" "+date+" "+type);
				...
				try {
					resp = restTemplate.getForObject(
							env.getUserUrl() + "rest-getHighRiskLocationWisecm?state=" + state + "&date=" + date + "&type=" + type,
							JsonResponse.class);
		...
		public @ResponseBody JsonResponse<Object> getTotalHealthCheckup(Model model, @RequestParam("state") String encodestate,
			...
			try {
				resp = restTemplate.getForObject(
						env.getUserUrl() + "rest-totalHealthCheckupcm?state=" + state + "&date=" + date + "&type=" + type,
						JsonResponse.class);
	...
	//Disese wise total patient no
		...
		public @ResponseBody JsonResponse<Object> getDiseaseWisePatientNo(Model model, @RequestParam("state") String encodestate,
			...
			try {
				resp = restTemplate.getForObject(
						env.getUserUrl() + "dieaseWise-patientnocm?state=" + state + "&date=" + date + "&type=" + type,
						JsonResponse.class);
	...
	//treanding disease
		...
		public @ResponseBody JsonResponse<Object> getreandingDisease(Model model, @RequestParam("state") String encodestate,
			...
			try {
				resp = restTemplate.getForObject(
						env.getUserUrl() + "treanding-diseasecm?state=" + state + "&date=" + date + "&type=" + type,
						JsonResponse.class);
	...
	//Count test details
		...
		public @ResponseBody JsonResponse<Object> getCountTestDetails(Model model, @RequestParam("state") String encodestate,
			...
			try {
				resp = restTemplate.getForObject(
						env.getUserUrl() + "rest-countTestDetailscm?state=" + state + "&date=" + date + "&type=" + type,
						JsonResponse.class);
